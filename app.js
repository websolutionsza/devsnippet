...
/* ============================================================
   ...
   ============================================================ */
// This creates a fake user so you can test the app without Google login
// ALL snippets will be saved under this user ID
const TEST_USER = {
    id: 'test-user-123',
    email: 'test@devsnippet.com',
    avatar: '',
...
async function fetchSnippets() {
    ...
    // ALWAYS filter by the test user ID so you only see your own snippets
...
async function createSnippet(payload) {
    const headers = getHeaders();
    const body = {
        user_id: state.user.id,  // ALWAYS set to the test user ID
        ...payload
    };
...
async function updateSnippet(id, payload) {
    const headers = getHeaders();
    // Security: filter by both id AND user_id (test user)
...
async function loadDashboard() {
    ...
    showPage('dashboard');
    $('user-email').textContent = user.email || 'Test User';
    if (user.avatar) {
...
/* ============================================================
   INIT - AUTO LOGIN AS TEST USER
   ============================================================ */
document.addEventListener('DOMContentLoaded', () => {
    // ============================================================
    // IMPORTANT: This automatically logs you in as a test user
    // ============================================================
    console.log('🔵 DevSnippet - Auto-login mode enabled');
    ...
    if (session) {
        ...
        console.log('✅ Existing session found for:', session.email);
        loadDashboard();
    } else {
        ...
        loadDashboard();
        showToast('Logged in as Test User (auto-login mode)', 'info');
    }
});

console.log('🚀 DevSnippet App Loaded (Auto-Login Mode)');
